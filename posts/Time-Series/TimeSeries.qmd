...
tsb_pref_cities <- preferred_cities %>%
                   ...
                   as_tsibble(key= c(RegionID, RegionName), index= Monthly_parsed)

tsb_pref_cities <-tsb_pref_cities%>%
  group_by_key() %>%
  index_by(year_month = ~ yearmonth(.)) # show the month name and year

tsb_pref_cities 
```
To visualize the data, I could usee the autoplot() command, but I rather to create my graph.

```{r fig.height= 10, message=FALSE, warning=FALSE}

plot_cities <- tsb_pref_cities %>%
